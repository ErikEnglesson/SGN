  ...
  def _create_process_example_fn(self) -> base.PreProcessFn:
    ...
    def _example_parser(example: types.Features) -> types.Features:
      ...
      if self._is_training:
        ...
        def weak_aug(image, seed1, seed2):
        ...
        if not self._aug_params.get('random_augment', False) and not use_augmix and self._aug_params['aug_count'] > 1:
          count = self._aug_params['aug_count']
          augment_seeds = tf.random.experimental.stateless_split(
            per_example_step_seeds[0], num=count*2)

          augmented = [
            weak_aug(image, augment_seeds[c*2],augment_seeds[c*2+1])
            for c in range(count)
          ]
          image = tf.stack(augmented)
        else:
          image = weak_aug(image, per_example_step_seeds[0], per_example_step_seeds[1])
...
'''
# Added by me
class Cifar10LabelCorruptedDataset(_CifarDataset):
  """CIFAR10 dataset builder class that adds noisy labels."""
  ...
  def __init__(
      ...
      severity: float = 0.4,
      **kwargs):
    """Create a CIFAR10 noisy label tf.data.Dataset builder.

    Args:
      corruption_type: Corruption name. e.g., sym, asym, instance
      severity: Corruption severity, as a fraction of mislabeled examples.
      **kwargs: Additional keyword arguments.
    """
    super().__init__(
        name=f'cifar10_LabelCorrupted/{corruption_type}_{severity}',
        fingerprint_key='id',
        **kwargs)  # pytype: disable=wrong-arg-types  # kwargs-checking
    # 
    assert corruption_type == 'asym'
    assert severity >= 0.0 and severity <= 1.0

  def _flip_label(example: types.Feaetures) -> types.Features:
    i = example['label']
    # truck -> automobile
    if i == 9:
        return 1
  ...
  def _create_noisy_label_fn() -> PreProcessFn:
      ...
      def _add_noisy_label(example: types.Features) -> types.Features:
        ...
        # Look up the corresponding noisy label
        # Add noisy label to example dict
        assert example['id'][:6] == 'train_'
        example_id = int(example['id'][6:])
        per_example_seed = tf.random.experimental.stateless_fold_in(self._seed, example_id) 
        random_func = functools.partial(stateless_random_ops.stateless_random_uniform,
                                        seed=per_example_seed)
        uniform_random = random_func(shape=[], minval=0, maxval=1.0)
        flip_cond = math_ops.less(uniform_random, self.severity)
        example['noisy_label'] = tf.cond(flip_cond, _flip_label, lambda: example['label'])
  ...
  def _create_process_example_fn(self) -> base.PreProcessFn:
      assert False
      fn = super()._create_process_example_fn()
      preprocess_fn = ops.compose(fn, _create_noisy_label_fn)
'''
