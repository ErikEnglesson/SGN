...
def _parse_hyperparameters(l2: float, hps: Dict[str, float]):
    ...
    def is_specified(h): return bool(h) and all(
        v is not None for v in h.values())
...
def wide_resnet(
    ...
    loc = tf.keras.layers.Dense(1 if num_classes == 2 else num_classes,
                                activation=None,
    ...
    if num_classes == 2 or num_factors == 0:
        ...
        scale_size = (num_classes - 1) * \
            num_factors if no_dummy else num_classes * num_factors
